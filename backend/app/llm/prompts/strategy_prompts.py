...
你的边界：
- 只规划下一步，不生成具体面试问题，不评价候选人，不给反馈
- 输出必须能被面试官直接执行：topic 要具体到场景/能力/证据，不要只写抽象维度名
- 决策优先级：追问必要性 > 维度覆盖缺口 > 岗位/导师匹配度 > 难度递进 > 话题新鲜度
...
规则6：避免重复，除非是追问
  → 若 topics_covered 已包含相同经历/项目/问题类型，continue 时应换维度或加深到新的证据点
  → 追问时 topic 必须说明要补齐的缺口，如"澄清个人贡献"、"补充量化结果"、"解释技术选型权衡"

规则7：研究生/导师面优先围绕导师研究方向匹配
  → 若岗位分析中包含导师研究方向，topic 应优先连接候选人的科研/项目经历与该方向
  → 不要只问泛泛的"为什么读研"；应问能验证研究基础、方法理解、论文/项目准备的问题"""
...
Boundaries:
- Plan only the next step; do not write the actual interview question, evaluate the candidate, or provide feedback
- The output must be directly actionable by the interviewer: topic should name a concrete scenario/skill/evidence need, not just an abstract dimension
- Decision priority: need to probe > uncovered dimensions > role/advisor fit > difficulty progression > topic novelty
...
Rule 6: Avoid repetition unless probing
  → If topics_covered already includes the same experience/project/question type, continue should switch dimensions or deepen toward a new evidence point
  → For probing, topic must name the missing evidence, e.g. "clarify personal contribution", "add quantified result", or "explain technical trade-off"

Rule 7: For graduate/advisor interviews, prioritize advisor research fit
  → If job analysis contains advisor research areas, topic should connect the candidate's research/project experience to those areas
  → Avoid only asking generic "why graduate school"; ask questions that test research foundation, methodology understanding, and paper/project preparation"""
...
def _phase_instruction_zh(question_count: int, max_questions: int, interview_type: str = "behavioral") -> str:
    if interview_type == "technical":
        if question_count <= 2:
            ...
            return "【技术面阶段：核心考察】穿插 technical_concept 与 algorithm/system_design，优先验证原理、复杂度、系统边界和技术选型理由；quantitative_probe 只用于项目指标追问。"
...
def _phase_instruction_en(question_count: int, max_questions: int, interview_type: str = "behavioral") -> str:
    if interview_type == "technical":
        if question_count <= 2:
            ...
            return "[Tech Interview Phase: Core Assessment] Mix technical_concept with algorithm/system_design. Prioritize principles, complexity, system boundaries, and technical trade-offs; use quantitative_probe only for project metric follow-ups."
        elif question_count >= max_questions - 2:
...
def _job_analysis_text_zh(job_analysis: dict) -> str:
    ...
    advisor_summary = job_analysis.get("advisor_research_summary")
    if not dims and not advisor_summary:
        return ""
